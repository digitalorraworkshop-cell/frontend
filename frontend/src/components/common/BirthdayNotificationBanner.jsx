                    ...
                    <div className="space-y-1.5">
                        {todayBirthdays.slice(0, 3).map(emp => (
                            <div key={emp._id} className="flex items-center gap-2 bg-white/15 rounded-xl px-3 py-2">
                                {emp.profilePicture ? (
                                    <img src={`${emp.profilePicture?.startsWith('http') ? emp.profilePicture : import.meta.env.VITE_API_URL + emp.profilePicture}`} alt={emp.name} className="w-6 h-6 rounded-full object-cover" />
                                ) : (
