...
const AdminEmployeeProfile = () => {
    ...
    const fetchEmployeeData = async () => {
        try {
            const [empRes, screenshotsRes, attendanceRes, leavesRes, assetsRes] = await Promise.all([
                api.get(`/employees/${id}`),
                api.get(`/screenshots?userId=${id}`),
                api.get(`/attendance/user/${id}`),
                api.get(`/leaves?userId=${id}`),
                api.get(`/assets/employee/${id}`)
            ]);
                    ...
                    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                        <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/20">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="text-lg font-bold text-slate-900">Assigned Fixed Assets</h3>
                            </div>
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Company Inventory</span>
                        </div>
                        <div className="p-8">
                                    ...
                                    assignedAssets.map((item, idx) => (
                                        <div key={idx} className="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl border border-slate-100">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="text-sm font-black text-slate-900">{item.product?.modelName}</p>
                                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Qty: {item.quantityAssigned}</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">SINCE</p>
                                                <p className="text-[11px] font-black text-slate-700">{new Date(item.distributionDate).toLocaleDateString()}</p>
                                            </div>
                                        </div>
                                    ))
                                )}
